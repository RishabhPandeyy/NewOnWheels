...
// MongoDB Connection
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://fallback_cluster.mongodb.net/mechway';
...
app.post('/api/contact', async (req, res) => {
  ...
  console.log("📝 POST /api/contact received:", { name, phone, subject, message });
  ...
  try {
    ...
    console.log("💾 Saving contact...", entry);
    await entry.save();
    console.log("✅ Contact saved successfully with ID:", entry._id);
...
app.post('/api/book', async (req, res) => {
  ...
  console.log("📝 POST /api/book received:", { name, phone, vehicleType, message });
  ...
  try {
    ...
    console.log("💾 Saving booking...", entry);
    await entry.save();
    console.log("✅ Booking saved successfully with ID:", entry._id);
    return res.json({ ok: true, id: entry._id });
  } catch (err) {
...
app.post('/api/inquiry', async (req, res) => {
  ...
  console.log("📝 POST /api/inquiry received:", { product_name, name, phone });
  ...
  try {
    ...
    console.log("💾 Saving inquiry...", entry);
    await entry.save();
    console.log("✅ Inquiry saved successfully with ID:", entry._id);
...
app.post('/api/payment', async (req, res) => {
  ...
  console.log("📝 POST /api/payment received:", { customerName, customerPhone, service, totalAmount });
  ...
  try {
    ...
    console.log("💾 Saving payment...", entry);
    await entry.save();
    console.log("✅ Payment saved successfully with ID:", entry._id);
    return res.json({ ok: true, id: entry._id });
  } catch (err) {
...
app.post('/api/signup', async (req, res) => {
  ...
  console.log("📝 POST /api/signup received:", { name, phone, email, address, aadharNo, mechanicType });
  ...
  try {
    ...
    if (existingUser) {
      console.warn("❌ User already exists with this phone, email, or aadharNo");
...
app.post('/api/login', async (req, res) => {
  ...
  console.log("📝 POST /api/login received with identifier:", identifier);
  
  if (!identifier || !password) {
    console.warn("❌ Validation failed - missing email/phone or password");
  ...
  try {
    ...
    if (!isPasswordValid) {
      console.warn("❌ Password mismatch for user:", identifier);
      return res.status(401).json({ error: 'Invalid email/phone or password' });
    }
...
app.post('/api/payment/verify', (req, res) => {
  ...
  if (expected === razorpay_signature) {
    ...
    console.warn('❌ Payment signature mismatch', { razorpay_order_id, razorpay_payment_id, razorpay_signature, expected });
...
app.listen(PORT, () => {
  ...
  console.log(`\n🚀 MechWay backend running at ${serverURL}`);
